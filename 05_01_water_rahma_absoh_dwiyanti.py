# -*- coding: utf-8 -*-
"""05_01_Water_Rahma Absoh Dwiyanti.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Ea1Z4wTzi-AFztcep-XyKELGX2xQtiP9
...
for i, col in enumerate(color):
  histr = cv2.calcHist(images=[lena],channels=[i],mask=None,histSize=[256], ranges=[0,256])
...
"""PyTorch and scikit-image are both popular libraries in the field of computer vision and image processing, but they serve different purposes and have different functionalities. PyTorch is primarily a deep learning framework, whereas scikit-image is an image processing library.

When it comes to max pooling operations, both PyTorch and scikit-image offer similar functionalities in terms of the mathematical operation performed (selecting the maximum value from a set of values). However, there are differences in how they are implemented and the context in which they are used
...
img_edited_min = block_reduce(img, block_size=BLOCK_SIZE1, func=np.min) # TODO: Recall the block_reduce(), but with func=np.min
plot(imgs=[img, img_edited_min])

img_edited_avg = block_reduce(img, block_size=BLOCK_SIZE1, func=np.mean).astype(int) # TODO: Recall the block_reduce().astype(int), but with func=np.mean
...
"""Min pooling selects the minimum value from a window of elements, preserving essential features and edges. It can be robust to small changes but sensitive to outliers. Average pooling calculates the mean, providing a smoothed representation and being less sensitive to outliers. Min pooling preserves edges, while average pooling smoothens data. The choice depends on the specific task and data characteristics. Max pooling captures significant features, average pooling smoothens, and min pooling is suitable for edge preservation and noise reduction
...
"""CLAHE and max pooling are image processing techniques with unique advantages. CLAHE enhances local contrast, especially in dark areas, preserving crucial details. It adapts to varying lighting conditions, reducing information loss, and improving overall image quality. Max pooling, on the other hand, is primarily used for down-sampling in neural networks. The choice between CLAHE and max pooling depends on specific task requirements and desired outcomes.

### Save your edited image
"""

FILE_NAME = "result_clahe.png" # TODO: Give any file name to your new CLAHE-enhanced photo, ends with .png extension
